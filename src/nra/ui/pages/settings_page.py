...
class SettingsPage(QWidget):
    ...
    def _init_ui(self):
        ...
        user_select_row.addWidget(QLabel("用户:"))
        ...
        self._save_status_label = QLabel("未检测")
        save_layout.addWidget(self._save_status_label)

        save_btn_row = QHBoxLayout()
        self._backup_btn = QPushButton("备份当前存档")
        ...
        backup_dir_row.addWidget(QLabel("备份目录:"))
        self._backup_dir_input = QLineEdit(self._settings.get("backup_dir", "data/save_backups"))
        ...
        backup_dir_browse = QPushButton("浏览")
    ...
    def _refresh_users(self):
        ...
        try:
            users = self._parse_loginusers_vdf(vdf_path)
            self._steam_users = users
            for uid, info in users.items():
                name = info.get("PersonaName", uid)
                recent = " [最近]" if info.get("MostRecent") == "1" else ""
    ...
    @staticmethod
    def _parse_loginusers_vdf(path: str) -> dict:
        """简易 VDF 解析"""
        import re
        users = {}
    ...
    def _on_backup(self):
        ...
        dest = os.path.join(backup_dir, f"{name.strip()}.sl2")
    ...
    def _refresh_backup_list(self):
        ...
        for name, fp, mtime, size in files:
            ...
            display = f"{name[:-4]}  |  {dt}  |  {size_mb:.1f} MB"
    ...
    def _on_restore(self):
        ...
        answer = QMessageBox.question(self, "确认恢复",
            "恢复将覆盖当前存档。\n恢复前会自动备份当前存档。\n确定继续？")
    ...
    def _on_rename_backup(self):
        ...
        new_path = os.path.join(os.path.dirname(old_path), f"{new_name.strip()}.sl2")
    ...
    def _on_delete_backup(self):
        ...
        answer = QMessageBox.question(self, "确认删除", "确定删除该备份？")
        if answer != QMessageBox.Yes:
            return
